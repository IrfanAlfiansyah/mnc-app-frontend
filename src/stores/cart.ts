  ...
  export const useCartStore = defineStore("cart", () => {
    ...
    const fetchCartItems = async (user_id: number) => {
      try {
        loading.value = true;
        error.value = null;
        const response = await fetch(`${VITE_API_BASE_URL}/cart/cart-items`, {
          method: "POST",
          headers: {
    ...
    const addToCart = async (
      ...
      try {
        ...
        const response = await fetch(`${VITE_API_BASE_URL}/cart/cart`, {
          ...
          body: JSON.stringify({ product_id, quantity, user_id, size, price }),
        });
    ...
    const updateCartItem = async (
      ...
      try {
        loading.value = true;
        error.value = null;
        const response = await fetch(`${VITE_API_BASE_URL}/cart/${cart_id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            // Jika menggunakan auth token:
